...
function tmplGetXferBar($total, $used, $title, $type='xfer') {
	...
	$total = (float)$total;
	$remaining = $total - ((float)$used / 1048576);
	$remaining = max(0, min($total, $remaining));
	$percent = ($total > 0) ? round(($remaining / $total) * 100, 0) : 0;
...
function getTorrentFluxLink() {
	global $cfg;
	$torrentFluxLink['address'] = "https://github.com/JawshTheDark/torrentflux-ng";
	$torrentFluxLink['name'] = "FluxTorrent ".$cfg["version"];
